...
void handleLogin(Message mess, int connSock) {
    ...
    if (numberElementsInArray(temp) == 3) {
        ...
        if ((numberElementsInArray(userStr) == 2) &&
            (numberElementsInArray(passStr) == 2)) { //check payload structure valid with two parameters
            if (!(strcmp(userStr[0], COMMAND_USER) ||
                  strcmp(passStr[0], COMMAND_PASSWORD))) { //check payload structure valid with two parameters
                    ...
                    if (loginCode != LOGIN_SUCCESS)
                        ...
                        if (mess.requestId == 0) {
                            ...
                            setClient(i, mess.requestId, username,
                                      connSock); // when user login success set user to online client
